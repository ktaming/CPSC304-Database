...
<body>
  ...
  <section>
    ...
    <body>
      ...
      <div style="text-align: center;">
        ...
        <hr />
        <h2>Insert Tuple into produce</h2>
        <form method="POST" action="produce.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="insertproduceRequest" name="insertproduceRequest">
  ...
  <div class="center"> -->
      <div style="text-align: center;">
        ...
        <h2>Select/List/Display tuples from produce</h2>
        <form method="GET" action="produce.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="displayproduceRequest" name="displayproduceRequest">

          <input type="submit" name="displayproduceTuples"></p>
        </form>
...
if (isset($_POST['insertproduceRequest'])) {
  handlePOSTRequest();
} else if (isset($_GET['countTupleRequest']) || isset($_GET['displayproduceRequest'])) {
  handleGETRequest();
}


function handleDisplayproduceList()
{
  global $db_conn;
  $result1 = executePlainSQL("SELECT COUNT(*) FROM produce");

  if (($row = oci_fetch_row($result1)) != false) {
    echo "<br> The number of tuples in produce: " . $row[0] . "<br>";
  }

  $result2 = executePlainSQL("SELECT * from produce");
  echo "<br>Retrieved data from table produce:<br>";
  echo "<table>";
  echo "<tr>
            <th>ARId</th>
            <th>WEId</th>
...
function tempDropproduce()
{
  global $db_conn;
  executePlainSQL("DROP TABLE produce");
  echo "<br> Dropped old produce table <br>";
}

function tempCreateproduce()
{
  global $db_conn;
  executePlainSQL("CREATE TABLE produce
      (
         ARId INTEGER,
         WEId CHAR(20),
         PRIMARY KEY (WEId),
  ...
  echo "<br> Created a new produce table <br>";
}



function handleInsertproduceRequest()
{
  global $db_conn;
  ...
  executeBoundSQL("insert into produce values (:bind1, :bind2)", $alltuples);
  OCICommit($db_conn);
}
...
{
  if (connectToDB()) {
    if (array_key_exists('insertproduceRequest', $_POST)) {
      tempDropproduce();
      tempCreateproduce();
      handleInsertproduceRequest();
      // This line is used for demonstration how Generic_produce_Example works. 你得先有个produce table，才能insert。
...
{
  if (connectToDB()) {
    if (array_key_exists('displayproduceTuples', $_GET)) {
      handleDisplayproduceList();
