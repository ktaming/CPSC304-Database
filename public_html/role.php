...
<body>
  ...
  <div class="center"> -->
      <div style="text-align: center;">
        ...
        <h2>Roles in Stark FAMILY who has multiple weapons</h2>
        <form method="GET" action="role.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="displayWeapon" name="displayWeapon">

          <input type="submit" name="displayWeapon"></p>
        </form>
  ...
  <div class="center"> -->
      <div style="text-align: center;">
        ...
        <h2>Select/List/Display tuples from ROLE</h2>
        <form method="GET" action="role.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="displayRoleRequest" name="displayRoleRequest">

          <input type="submit" name="displayRoleTuples"></p>
        </form>
  ...
  if (isset($_POST['insertRoleRequest'])) {
    handlePOSTRequest();
  } else if (isset($_GET['countTupleRequest']) || isset($_GET['displayRoleRequest']) || isset($_GET['displayWeapon'])) {
    handleGETRequest();
  } 
  ...
  {
    global $db_conn;
    $result = executePlainSQL("SELECT  ROName FROM  ROLE
                                        WHERE  FAName = 'Stark'
                                        INTERSECT
                                        SELECT  ROName
                                        FROM  WEAPON
                                        GROUP BY  ROName
                                        HAVING  COUNT(*) >= 2");
    echo "<br>Retrieved the role name of who owns multiple weapons:<br>";
    echo "<table>";
    echo "<tr>
            <th>ROName</th>
            </tr>";
  ...
  function handleDisplayRoleList()
  {
    global $db_conn;
    $result1 = executePlainSQL("SELECT COUNT(*) FROM ROLE");

    if (($row = oci_fetch_row($result1)) != false) {
      echo "<br> The number of tuples in role: " . $row[0] . "<br>";
    }

    $result2 = executePlainSQL("SELECT * from ROLE");
    echo "<br>Retrieved data from table role:<br>";
    echo "<table>";
    echo "<tr>
            <th>ROName</th>
            <th>Age</th>
  ...
  function handleInsertRoleRequest()
  {
    ...
    $tuple = array(
      ...
      ":bind2" => $_POST['insAge'],
      ":bind3" => $_POST['insGender'],
      ":bind4" => $_POST['insFAName'],
      ":bind5" => $_POST['insNAName'],
      ":bind6" => $_POST['insTitle'],
      ":bind7" => $_POST['insLeader']
    );
    ...
    executeBoundSQL("insert into ROLE values (:bind1, :bind2, :bind3, :bind4, :bind5, :bind6, :bind7,)", $alltuples);
    OCICommit($db_conn);
  }
  ...
  {
    if (connectToDB()) {
      if (array_key_exists('insertRoleRequest', $_POST)) {
        // tempDropArmy();
        // tempCreateArmy();
  ...
  {
    if (connectToDB()) {
      if (array_key_exists('displayRoleTuples', $_GET)) {
        handleDisplayRoleList();
        //                } else if (array_key_exists('countTuples', $_GET)) {
        //                    handleDisRequest();
      } else if (array_key_exists('displayWeapon', $_GET)) {
        handleWeapon();
      }
