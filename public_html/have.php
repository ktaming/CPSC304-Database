...
<body>
    ...
    <div style="text-align: center;">
      ...
      <hr />
        <h2>Insert Tuple into have</h2>
        <form method="POST" action="have.php"> <!--refresh page when submitted-->
            <input type="hidden" id="inserthaveRequest" name="inserthaveRequest">
...
<div style="text-align: center;">
    ...
    <h2>Select/List/Display tuples from have</h2>
        <form method="GET" action="have.php"> <!--refresh page when submitted-->
            <input type="hidden" id="displayhaveRequest" name="displayhaveRequest">
            
            <input type="submit" name="displayhaveTuples"></p>
        </form>
...
<?php
		...
		if (isset($_POST['inserthaveRequest'])) {
            handlePOSTRequest();
        } else if (isset($_GET['countTupleRequest']) || isset($_GET['displayhaveRequest'])) {
            handleGETRequest();
        }
		
		
		function handleDisplayhaveList() {
            global $db_conn;
            $result1 = executePlainSQL("SELECT COUNT(*) FROM have");

            if (($row = oci_fetch_row($result1)) != false) {
                echo "<br> The number of tuples in have: " . $row[0] . "<br>";
            }
        
            $result2 = executePlainSQL("SELECT * from have");
			echo "<br>Retrieved data from table have:<br>";
            echo "<table>";
            echo "<tr>
            <th>FAName</th>
            <th>ARId</th>
		...
		function tempDrophave() {
			global $db_conn;
			executePlainSQL("DROP TABLE have");
            echo "<br> Dropped old have table <br>";
		}
		
		function tempCreatehave() {
      global $db_conn;
      executePlainSQL("CREATE TABLE have (
        FAName      CHAR(20),
        Flag  CHAR(20),
          Mascot CHAR(20),");
            echo "<br> Created a new have table <br>";
    }
      
			

		function handleInserthaveRequest() {
            global $db_conn;
            ...
            executeBoundSQL("insert into have values (:bind1, :bind2)", $alltuples);
            OCICommit($db_conn);
        }
        ...
        function handlePOSTRequest() {
            if (connectToDB()) {
                if (array_key_exists('inserthaveRequest', $_POST)) {
					// tempDrophave();
					// tempCreatehave();
                    handleInserthaveRequest();
					// This line is used for demonstration how Generic_have_Example works. 你得先有个have table，才能insert。
...
//                    handleInsertRequest();
        ...
        function handleGETRequest() {
            if (connectToDB()) {
                if (array_key_exists('displayhaveTuples', $_GET)) {
                    handleDisplayhaveList();
