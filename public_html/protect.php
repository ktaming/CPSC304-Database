...
<body>
    ...
    <div style="text-align: center;">
      ...
      <hr />
        <h2>Insert Tuple into protect</h2>
        <form method="POST" action="protect.php"> <!--refresh page when submitted-->
            <input type="hidden" id="insertprotectRequest" name="insertprotectRequest">
...
<div style="text-align: center;">
    ...
    <h2>Select/List/Display tuples from protect</h2>
        <form method="GET" action="protect.php"> <!--refresh page when submitted-->
            <input type="hidden" id="displayprotectRequest" name="displayprotectRequest">
            
            <input type="submit" name="displayprotectTuples"></p>
        </form>
...
<?php
		...
		if (isset($_POST['insertprotectRequest'])) {
            handlePOSTRequest();
        } else if (isset($_GET['countTupleRequest']) || isset($_GET['displayprotectRequest'])) {
            handleGETRequest();
        }
		
		
		function handleDisplayprotectList() {
            global $db_conn;
            $result1 = executePlainSQL("SELECT COUNT(*) FROM protect");

            if (($row = oci_fetch_row($result1)) != false) {
                echo "<br> The number of tuples in protect: " . $row[0] . "<br>";
            }
        
            $result2 = executePlainSQL("SELECT * from protect");
			echo "<br>Retrieved data from table protect:<br>";
            echo "<table>";
            echo "<tr>
            <th>ARName</th>
            <th>NAName</th>
		...
		function tempDropprotect() {
			global $db_conn;
			executePlainSQL("DROP TABLE protect");
            echo "<br> Dropped old protect table <br>";
		}
		
		function tempCreateprotect() {
      global $db_conn;
      ...
      (
         ...
         FOREIGN KEY (NAName) REFERENCES NATION (NAName)
            ...
            echo "<br> Created a new protect table <br>";
    }
      
			

		function handleInsertprotectRequest() {
            global $db_conn;

            //Getting the values from user and insert data into the table
            $tuple = array (
                ":bind1" => $_POST['insARName'],
                ":bind2" => $_POST['insNAname'],
            );
            ...
            executeBoundSQL("insert into protect values (:bind1, :bind2)", $alltuples);
            OCICommit($db_conn);
        }
        ...
        function handlePOSTRequest() {
            if (connectToDB()) {
                if (array_key_exists('insertprotectRequest', $_POST)) {
					// tempDropprotect();
					// tempCreateprotect();
                    handleInsertprotectRequest();
					// This line is used for demonstration how Generic_protect_Example works. 你得先有个protect table，才能insert。
...
//                    handleInsertRequest();
        ...
        function handleGETRequest() {
            if (connectToDB()) {
                if (array_key_exists('displayprotectTuples', $_GET)) {
                    handleDisplayprotectList();
