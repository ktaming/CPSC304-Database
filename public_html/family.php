...
<body>
  ...
  <section>
    ...
    <body>
      ...
      <div style="text-align: center;">
        ...
        <hr />
        <h2>Insert a Family</h2>
        <form method="POST" action="family.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="insertFamilyRequest" name="insertFamilyRequest">
          FAName: <input type="text" name="insFAName"> <br /><br />
          Flag: <input type="text" name="insFlag"> <br /><br />
          Mascot: <input type="text" name="insMascot"> <br /><br />
  ...
  <section>
    ...
    <body>
      <div style="text-align: center;">
        <hr />
        <h2>Delete a Family</h2>
        <form method="POST" action="family.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="deleteFamilyRequest" name="deleteFamilyRequest">
          FAName: <input type="text" name="delFAName"> <br /><br />
  ...
  <div class="center"> -->
      <div style="text-align: center;">
        ...
        <h2>Select/List/Display tuples from Family</h2>
        <form method="GET" action="family.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="displayFamilyRequest" name="displayFamilyRequest">

          <input type="submit" name="displayFamilyTuples"></p>
        </form>
...
if (isset($_POST['insertFamilyRequest']) || isset($_POST['deleteFamilyRequest'])) {
  handlePOSTRequest();
} else if (isset($_GET['countTupleRequest']) || isset($_GET['displayFamilyRequest'])) {
  handleGETRequest();
}
...
function handleDeleteRequest()
{
  ...
  $faname =  $_POST['delFAName'];
  
  executePlainSQL("DELETE FROM family WHERE FAName = '" . $faname . "' ");
  OCICommit($db_conn);
}


function handleDisplayFamilyList()
{
  global $db_conn;
  $result1 = executePlainSQL("SELECT COUNT(*) FROM FAMILY");

  if (($row = oci_fetch_row($result1)) != false) {
    echo "<br> The number of tuples in family: " . $row[0] . "<br>";
  }

  $result2 = executePlainSQL("SELECT * from family");
  echo "<br>Retrieved data from table family:<br>";
  echo "<table>";
  echo "<tr>
            <th>FAName</th>
            <th>Flag</th>
...
function tempDropFamily()
{
  global $db_conn;
  executePlainSQL("DROP TABLE FAMILY");
  echo "<br> Dropped old family table <br>";
}

function tempCreateFamily()
{
  global $db_conn;
  executePlainSQL("CREATE TABLE FAMILY (
        FAName      CHAR(20),
        Flag  CHAR(20),
          Mascot CHAR(20),");
  echo "<br> Created a new family table <br>";
}



function handleInsertFamilyRequest()
{
  global $db_conn;

  //Getting the values from user and insert data into the table
  $tuple = array(
    ":bind1" => $_POST['insFAName'],
    ":bind2" => $_POST['insFlag'],
    ":bind3" => $_POST['insMascot'],
  );
...
{
  if (connectToDB()) {
    if (array_key_exists('insertFamilyRequest', $_POST)) {
      // tempDropFamily();
      // tempCreateFamily();
      handleInsertFamilyRequest(); 
    } else if (array_key_exists('deleteFamilyRequest', $_POST)) {
      // tempDropFamily();
      // tempCreateFamily();
      handleDeleteRequest(); 
    }
...
{
  if (connectToDB()) {
    if (array_key_exists('displayFamilyTuples', $_GET)) {
      handleDisplayFamilyList();
