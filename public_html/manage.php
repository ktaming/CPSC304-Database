...
<body>
    ...
    <div style="text-align: center;">
      ...
      <hr />
        <h2>Insert Tuple into manage</h2>
        <form method="POST" action="manage.php"> <!--refresh page when submitted-->
            <input type="hidden" id="insertmanageRequest" name="insertmanageRequest">
            fid: <input type="text" name="insFid"> <br /><br />
            nid: <input type="text" name="insNid"> <br /><br />
...
<div style="text-align: center;">
    ...
    <h2>Select/List/Display tuples from manage</h2>
        <form method="GET" action="manage.php"> <!--refresh page when submitted-->
            <input type="hidden" id="displaymanageRequest" name="displaymanageRequest">
            
            <input type="submit" name="displaymanageTuples"></p>
        </form>
...
<?php
		...
		if (isset($_POST['insertmanageRequest'])) {
            handlePOSTRequest();
        } else if (isset($_GET['countTupleRequest']) || isset($_GET['displaymanageRequest'])) {
            handleGETRequest();
        }
		
		
		function handleDisplaymanageList() {
            global $db_conn;
            $result1 = executePlainSQL("SELECT COUNT(*) FROM manage");

            if (($row = oci_fetch_row($result1)) != false) {
                echo "<br> The number of tuples in manage: " . $row[0] . "<br>";
            }
        
            $result2 = executePlainSQL("SELECT * from manage");
			echo "<br>Retrieved data from table manage:<br>";
            echo "<table>";
            echo "<tr>
            <th>Fid</th>
            <th>Nid</th>
		...
		function tempDropmanage() {
			global $db_conn;
			executePlainSQL("DROP TABLE manage");
            echo "<br> Dropped old manage table <br>";
		}
		
		function tempCreatemanage() {
      global $db_conn;
      executePlainSQL("
      CREATE TABLE manage
      (
         ...
         FOREIGN KEY (nid) REFERENCES NATION (NAName)
            ...
            echo "<br> Created a new manage table <br>";
    }
      
			

		function handleInsertmanageRequest() {
            global $db_conn;
            ...
            executeBoundSQL("insert into manage values (:bind1, :bind2)", $alltuples);
            OCICommit($db_conn);
        }
        ...
        function handlePOSTRequest() {
            if (connectToDB()) {
                if (array_key_exists('insertmanageRequest', $_POST)) {
					// tempDropmanage();
					// tempCreatemanage();
                    handleInsertmanageRequest();
					// This line is used for demonstration how Generic_manage_Example works. 你得先有个manage table，才能insert。
...
//                    handleInsertRequest();
        ...
        function handleGETRequest() {
            if (connectToDB()) {
                if (array_key_exists('displaymanageTuples', $_GET)) {
                    handleDisplaymanageList();
