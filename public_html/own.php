...
<body>
  ...
  <section>
    ...
    <body>
      ...
      <div style="text-align: center;">
        ...
        <hr />
        <h2>Insert Tuple into own</h2>
        <form method="POST" action="own.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="insertownRequest" name="insertownRequest">
  ...
  <div class="center"> -->
      <div style="text-align: center;">
        ...
        <h2>Select/List/Display tuples from own</h2>
        <form method="GET" action="own.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="displayownRequest" name="displayownRequest">

          <input type="submit" name="displayownTuples"></p>
        </form>
...
if (isset($_POST['insertownRequest'])) {
  handlePOSTRequest();
} else if (isset($_GET['countTupleRequest']) || isset($_GET['displayownRequest'])) {
  handleGETRequest();
}


function handleDisplayownList()
{
  global $db_conn;
  $result1 = executePlainSQL("SELECT COUNT(*) FROM own");

  if (($row = oci_fetch_row($result1)) != false) {
    echo "<br> The number of tuples in own: " . $row[0] . "<br>";
  }

  $result2 = executePlainSQL("SELECT * from own");
...
function tempDropown()
{
  global $db_conn;
  executePlainSQL("DROP TABLE own");
  echo "<br> Dropped old own table <br>";
}

function tempCreateown()
{
  global $db_conn;
  executePlainSQL("CREATE TABLE own
      (
         ROName CHAR(20),
         WEId   CHAR(20),
         PRIMARY KEY (WEId),
  ...
  echo "<br> Created a new own table <br>";
}



function handleInsertownRequest()
{
  global $db_conn;
  ...
  executeBoundSQL("insert into own values (:bind1, :bind2)", $alltuples);
  OCICommit($db_conn);
}
...
{
  if (connectToDB()) {
    if (array_key_exists('insertownRequest', $_POST)) {
      // tempDropown();
      // tempCreateown();
      handleInsertownRequest();
      // This line is used for demonstration how Generic_own_Example works. 你得先有个own table，才能insert。
...
{
  if (connectToDB()) {
    if (array_key_exists('displayownTuples', $_GET)) {
      handleDisplayownList();
