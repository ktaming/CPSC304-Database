...
<body>
    ...
    <div style="text-align: center;">
      ...
      <hr />
        <h2>Insert Tuple into belong</h2>
        <form method="POST" action="belong.php"> <!--refresh page when submitted-->
            <input type="hidden" id="insertbelongRequest" name="insertbelongRequest">
            rid: <input type="text" name="insFAName"> <br /><br />
            fid: <input type="text" name="insFlag"> <br /><br />
...
<div style="text-align: center;">
    ...
    <h2>Select/List/Display tuples from belong</h2>
        <form method="GET" action="belong.php"> <!--refresh page when submitted-->
            <input type="hidden" id="displaybelongRequest" name="displaybelongRequest">
            
            <input type="submit" name="displaybelongTuples"></p>
        </form>
...
<?php
		...
		if (isset($_POST['insertbelongRequest'])) {
            handlePOSTRequest();
        } else if (isset($_GET['countTupleRequest']) || isset($_GET['displaybelongRequest'])) {
            handleGETRequest();
        }
		
		
		function handleDisplaybelongList() {
            global $db_conn;
            $result1 = executePlainSQL("SELECT COUNT(*) FROM belong");

            if (($row = oci_fetch_row($result1)) != false) {
                echo "<br> The number of tuples in belong: " . $row[0] . "<br>";
            }
        
            $result2 = executePlainSQL("SELECT * from belong");
			echo "<br>Retrieved data from table belong:<br>";
            echo "<table>";
            echo "<tr>
            <th>Rid</th>
            <th>Fid</th>
		...
		function tempDropbelong() {
			global $db_conn;
			executePlainSQL("DROP TABLE belong");
            echo "<br> Dropped old belong table <br>";
		}
		
		function tempCreatebelong() {
      global $db_conn;
      executePlainSQL("CREATE TABLE belong
      (
         rid CHAR(20),
         fid CHAR(20),
         PRIMARY KEY (rid),
         ...
         FOREIGN KEY (fid) REFERENCES FAMILY (FAName) NOT DEFERRABLE
            ...
            echo "<br> Created a new belong table <br>";
    }
      
			

		function handleInsertbelongRequest() {
            global $db_conn;
            ...
            executeBoundSQL("insert into belong values (:bind1, :bind2)", $alltuples);
            OCICommit($db_conn);
        }
        ...
        function handlePOSTRequest() {
            if (connectToDB()) {
                if (array_key_exists('insertbelongRequest', $_POST)) {
					// tempDropbelong();
					// tempCreatebelong();
                    handleInsertbelongRequest();
					// This line is used for demonstration how Generic_belong_Example works. 你得先有个belong table，才能insert。
...
//                    handleInsertRequest();
        ...
        function handleGETRequest() {
            if (connectToDB()) {
                if (array_key_exists('displaybelongTuples', $_GET)) {
                    handleDisplaybelongList();
