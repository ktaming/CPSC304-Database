...
<body>
    ...
    <div style="text-align: center;">
      ...
      <hr />
        <h2>Insert Tuple into join</h2>
        <form method="POST" action="join.php"> <!--refresh page when submitted-->
            <input type="hidden" id="insertjoinRequest" name="insertjoinRequest">
            ALName: <input type="text" name="insALName"> <br /><br />
            FAName: <input type="text" name="insFAName"> <br /><br />
            Leader: <input type="text" name="insLeader"> <br /><br />
...
<div style="text-align: center;">
    ...
    <h2>Select/List/Display tuples from join</h2>
        <form method="GET" action="join.php"> <!--refresh page when submitted-->
            ...
            <input type="submit" name="displayjoinTuples"></p>
        </form>
...
<?php
		...
		if (isset($_POST['insertjoinRequest'])) {
            handlePOSTRequest();
        } else if (isset($_GET['countTupleRequest']) || isset($_GET['displayjoinRequest'])) {
            handleGETRequest();
        }
		
		
		function handleDisplayjoinList() {
            global $db_conn;
            $result1 = executePlainSQL("SELECT COUNT(*) FROM join");

            if (($row = oci_fetch_row($result1)) != false) {
                echo "<br> The number of tuples in join: " . $row[0] . "<br>";
            }
        
            $result2 = executePlainSQL("SELECT * from join");
			echo "<br>Retrieved data from table join:<br>";
            echo "<table>";
            echo "<tr>
            <th>ALName</th>
            <th>FAName</th>
		...
		function tempDropjoin() {
			global $db_conn;
			executePlainSQL("DROP TABLE join");
            echo "<br> Dropped old join table <br>";
		}
		
		function tempCreatejoin() {
      global $db_conn;
      executePlainSQL("CREATE TABLE join
      (
         ALName CHAR(20),
         FAName CHAR(20),
         Leader CHAR(20),
         ...
         FOREIGN KEY (ALName) REFERENCES ALLIANCE (ALName)
            ...
            echo "<br> Created a new join table <br>";
    }
      
			

		function handleInsertjoinRequest() {
            global $db_conn;

            //Getting the values from user and insert data into the table
            $tuple = array (
                ":bind1" => $_POST['insALName'],
                ":bind2" => $_POST['insFAName'],
                ":bind3" => $_POST['insLeader'],
            );
            ...
            executeBoundSQL("insert into join values (:bind1, :bind2, :bind3)", $alltuples);
            OCICommit($db_conn);
        }
        ...
        function handlePOSTRequest() {
            if (connectToDB()) {
                if (array_key_exists('insertjoinRequest', $_POST)) {
					// tempDropjoin();
					// tempCreatejoin();
                    handleInsertjoinRequest();
					// This line is used for demonstration how Generic_join_Example works. 你得先有个join table，才能insert。
...
//                    handleInsertRequest();
        ...
        function handleGETRequest() {
            if (connectToDB()) {
                if (array_key_exists('displayjoinTuples', $_GET)) {
                    handleDisplayjoinList();
