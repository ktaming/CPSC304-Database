...
<body>
  ...
  <section>
    ...
    <body>
      ...
      <div style="text-align: center;">
        ...
        <hr />
        <h2>Insert Tuple into isFrom</h2>
        <form method="POST" action="from.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="insertisFromRequest" name="insertisFromRequest">
          rid: <input type="text" name="insFAName"> <br /><br />
          nid: <input type="text" name="insFlag"> <br /><br />
  ...
  <div class="center"> -->
      <div style="text-align: center;">
        ...
        <h2>Select/List/Display tuples from isFrom</h2>
        <form method="GET" action="from.php">
          <!--refresh page when submitted-->
          <input type="hidden" id="displayisFromRequest" name="displayisFromRequest">

          <input type="submit" name="displayisFromTuples"></p>
        </form>
...
if (isset($_POST['insertisFromRequest'])) {
  handlePOSTRequest();
} else if (isset($_GET['countTupleRequest']) || isset($_GET['displayisFromRequest'])) {
  handleGETRequest();
}


function handleDisplayisFromList()
{
  global $db_conn;
  $result1 = executePlainSQL("SELECT COUNT(*) FROM isFrom");

  if (($row = oci_fetch_row($result1)) != false) {
    echo "<br> The number of tuples in isFrom: " . $row[0] . "<br>";
  }

  $result2 = executePlainSQL("SELECT * from isFrom");
  echo "<br>Retrieved data from table isFrom:<br>";
  echo "<table>";
  echo "<tr>
            <th>Rid</th>
            <th>Nid</th>
...
function tempDropisFrom()
{
  global $db_conn;
  executePlainSQL("DROP TABLE isFrom");
  echo "<br> Dropped old isFrom table <br>";
}

function tempCreateisFrom()
{
  global $db_conn;
  executePlainSQL("CREATE TABLE isFrom
      (
         rid CHAR(20),
         nid CHAR(20),
         PRIMARY KEY (rid),
  ...
  echo "<br> Created a new isFrom table <br>";
}



function handleInsertisFromRequest()
{
  global $db_conn;
  ...
  executeBoundSQL("insert into isFrom values (:bind1, :bind2)", $alltuples);
  OCICommit($db_conn);
}
...
{
  if (connectToDB()) {
    if (array_key_exists('insertisFromRequest', $_POST)) {
      // tempDropisFrom();
      // tempCreateisFrom();
      handleInsertisFromRequest();
      // This line is used for demonstration how Generic_isFrom_Example works. 你得先有个isFrom table，才能insert。
...
{
  if (connectToDB()) {
    if (array_key_exists('displayisFromTuples', $_GET)) {
      handleDisplayisFromList();
